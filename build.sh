...
docker build --build-arg ELASTICSEARCH_VERSION=${_elasticsearch_version} --tag "stakater/elasticsearch-kubernetes:${_elasticsearch_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker build --build-arg ELASTICSEARCH_VERSION=${_elasticsearch_version} --tag "stakater/elasticsearch-kubernetes:latest"  --no-cache=true .
fi